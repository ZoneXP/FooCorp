...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create branch" do
    assert_difference('Branch.count') do
      post branches_url, params: { branch: { address: @branch.address, id_branch: @branch.id_branch, name: @branch.name } }
    end
  ...
  test "should update branch" do
    patch branch_url(@branch), params: { branch: { address: @branch.address, id_branch: @branch.id_branch, name: @branch.name } }
